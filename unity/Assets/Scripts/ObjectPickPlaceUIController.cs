...
namespace UnityStandardAssets.Characters.FirstPerson {
    ...
    public class ObjectPickPlaceUIController {
        [SerializeField]
        ...
        private SimObjPhysics pickedUpObject;
        ...
        public ObjectPickPlaceUIController(
        ...
        public void MouseControls(Vector3 screenPosition) {
            // Interact action for mouse left-click when nothing is picked up
            if (Input.GetKeyDown(KeyCode.Mouse0)) {
                Debug.Log(this.PhysicsController.WhatAmIHolding());
                if (
                    ...
                    GameObject indicator = GameObject.Find("ObjectPlacementIndicator");
                    Vector3 original = this.pickedUpObject.gameObject.transform.position;
                    Vector3 target = indicator.transform.position;
                    target.y += 0.1f;
                    
                    Dictionary<string, object> action = new Dictionary<string, object>();
                    action["action"] = "PlaceObjectAtPoint";
                    action["objectId"] = this.pickedUpObject.objectID;
                    ...
                    indicator.transform.position = new Vector3(0, -10000, 0);
        ...
        public void UpdateHighlightedObject(Vector3 screenPosition) {
            ...
            if (!pickupState) {
                ...
                // Calculate how close the normal is to up, i.e. how flat the surface is
                float dot = Vector3.Dot(hit.normal, Vector3.up);

                // Example threshold: 0.9 means within ~25.8 degrees of flat
                GameObject indicator = GameObject.Find("ObjectPlacementIndicator");
                if (dot >= 0.9f) {
                    // Move the indicator there and show it
