...
public class RemoteAssetPrefabGenerator : EditorWindow {
    ...
    public string PrefabFolder = "Assets/Prefabs/ProceduralAssets";

    [MenuItem("Tools/Generate Procedural Asset Prefabs")]
    public static void ShowWindow() {
        GetWindow<RemoteAssetPrefabGenerator>("Procedural Prefab Generator");
    }

    private void OnGUI() {
        EditorGUILayout.LabelField("Procedural Asset Prefab Generator", EditorStyles.boldLabel);

        AssetBase = EditorGUILayout.TextField("Asset Base URL", AssetBase);
        ...
        if (GUILayout.Button("Generate Prefabs")) {
            ...
            GeneratePrefabsAsync().Forget(); // We'll use UniTask-like Fire-and-Forget helper
        }

        if (GUILayout.Button("Assign Prefabs & Materials")) {
            AssignPrefabsAndMaterials();
        }
    }

    private void AssignPrefabsAndMaterials() {
        // Find ProceduralManager and its database
        var managerGO = GameObject.Find("ProceduralManager");
        if (managerGO == null) {
            Debug.LogError("Could not find GameObject 'ProceduralManager'.");
            return;
        }

        var database = managerGO.GetComponent<ProceduralAssetDatabase>();
        if (database == null) {
            Debug.LogError("ProceduralManager does not have a ProceduralAssetDatabase component.");
            return;
        }
        ...
        string[] materialGuids = AssetDatabase.FindAssets("t:Material", new[] { PrefabFolder });
        ...
        // Update totalMats
        database.totalMats = database.materials.Count;
        ...
        Debug.Log($"Assigned {database.prefabs.Count} prefabs and {database.materials.Count} materials to ProceduralAssetDatabase.");
    }

    Texture2D MakeTextureAsset(Texture2D source) {
        if (source == null) return null;

        // Create a new texture with the same dimensions and format
        Texture2D texCopy = new Texture2D(source.width, source.height, TextureFormat.RGBA32, source.mipmapCount > 1);
        texCopy.SetPixels(source.GetPixels()); // copy pixel data
        texCopy.Apply();

        return texCopy;
    }

    private async Task GeneratePrefabsAsync() {
        foreach (var assetId in AssetIds) {
            try {
                string prefabPath = Path.Combine(PrefabFolder, $"{assetId}.prefab");
                // --- Skip if prefab already exists ---
                if (File.Exists(prefabPath)) {
                    Debug.Log($"Prefab already exists for {assetId}, skipping.");
                    continue;
                }

                Debug.Log($"Loading asset {assetId}...");
                // Load the procedural asset JSON
                var procAsset = await RemoteAssetManager.LoadAssetFromUrlAsync(
                    $"{AssetBase}/{assetId}/{assetId}.json"
                );

                var loadResult = await RemoteAssetManager.CreateRuntimeAsset(procAsset, AssetBase, returnObject: true);
                if (!loadResult.success) {
                    Debug.LogError($"Failed to load asset {assetId}: {loadResult.errorMessage}");
                    continue;
                }

                var agentManager = FindObjectOfType<AgentManager>();
                var controller = agentManager.PrimaryAgent as PhysicsRemoteFPSAgentController;
                if (controller == null) {
                    Debug.LogError("PrimaryAgent is null or not a PhysicsRemoteFPSAgentController!");
                    continue;
                }

                var spawnResult = controller.SpawnAsset(assetId, assetId);
                if (!spawnResult.success) {
                    Debug.LogError($"Failed to spawn asset {assetId}: {spawnResult.errorMessage}");
                    continue;
                }

                GameObject spawnedObject = GameObject.Find(assetId);
                if (spawnedObject == null) {
                    Debug.LogError($"SpawnAsset did not return a GameObject for {assetId}");
                    continue;
                }

                // --- Folders ---
                string meshFolder = Path.Combine(PrefabFolder, "Meshes");
                string matFolder = Path.Combine(PrefabFolder, "Materials");
                string texFolder = Path.Combine(PrefabFolder, "Textures");
                ...
                // --- Save Meshes ---
                foreach (var mf in spawnedObject.GetComponentsInChildren<MeshFilter>()) {
                    if (mf.sharedMesh == null) continue;
                    Mesh meshCopy = UnityEngine.Object.Instantiate(mf.sharedMesh);
                    meshCopy.name = $"{assetId}_{mf.name}";
                    string meshPath = Path.Combine(meshFolder, $"{meshCopy.name}.asset");
                    AssetDatabase.CreateAsset(meshCopy, meshPath);
                    mf.sharedMesh = meshCopy;
                }

                // --- Save MeshColliders ---
                foreach (var col in spawnedObject.GetComponentsInChildren<MeshCollider>()) {
                    if (col.sharedMesh == null) continue;
                    Mesh meshCopy = UnityEngine.Object.Instantiate(col.sharedMesh);
                    meshCopy.name = $"{assetId}_{col.name}_collider";
                    string meshPath = Path.Combine(meshFolder, $"{meshCopy.name}.asset");
                    AssetDatabase.CreateAsset(meshCopy, meshPath);
                    col.sharedMesh = meshCopy;
                }

                // --- Save Materials & Textures ---
                foreach (var r in spawnedObject.GetComponentsInChildren<MeshRenderer>()) {
                    ...
                    for (int i = 0; i < r.sharedMaterials.Length; i++) {
                        var srcMat = r.sharedMaterials[i];
                        if (srcMat == null) continue;

                        Material matCopy = new Material(srcMat);
                        matCopy.name = $"{assetId}_{srcMat.name}";

                        // Albedo
                        if (srcMat.HasProperty("_MainTex")) {
                            var tex = srcMat.GetTexture("_MainTex") as Texture2D;
                            if (tex != null) {
                                Texture2D texCopy = MakeTextureAsset(tex);
                                string texPath = Path.Combine(texFolder, $"{assetId}_{srcMat.name}_Albedo.asset");
                                AssetDatabase.CreateAsset(texCopy, texPath);
                                matCopy.SetTexture("_MainTex", texCopy);
                            }
                        }

                        // Metallic
                        if (srcMat.HasProperty("_MetallicGlossMap")) {
                            var tex = srcMat.GetTexture("_MetallicGlossMap") as Texture2D;
                            if (tex != null) {
                                Texture2D texCopy = MakeTextureAsset(tex);
                                string texPath = Path.Combine(texFolder, $"{assetId}_{srcMat.name}_Metallic.asset");
                                AssetDatabase.CreateAsset(texCopy, texPath);
                                matCopy.SetTexture("_MetallicGlossMap", texCopy);
                            }
                        }

                        // Normal map
                        if (srcMat.HasProperty("_BumpMap")) {
                            var tex = srcMat.GetTexture("_BumpMap") as Texture2D;
                            if (tex != null) {
                                Texture2D texCopy = MakeTextureAsset(tex);
                                string texPath = Path.Combine(texFolder, $"{assetId}_{srcMat.name}_Normal.asset");
                                AssetDatabase.CreateAsset(texCopy, texPath);
                                matCopy.SetTexture("_BumpMap", texCopy);
                            }
                        }

                        // Emission
                        if (srcMat.HasProperty("_EmissionMap")) {
                            var tex = srcMat.GetTexture("_EmissionMap") as Texture2D;
                            if (tex != null) {
                                Texture2D texCopy = MakeTextureAsset(tex);
                                string texPath = Path.Combine(texFolder, $"{assetId}_{srcMat.name}_Emission.asset");
                                AssetDatabase.CreateAsset(texCopy, texPath);
                                matCopy.SetTexture("_EmissionMap", texCopy);
                            }
                        }

                        string matPath = Path.Combine(matFolder, $"{matCopy.name}.mat");
                ...
                // --- Save Prefab ---
                PrefabUtility.SaveAsPrefabAsset(spawnedObject, prefabPath);
                Debug.Log($"Prefab created at {prefabPath}");

                // Optional: destroy runtime object after saving prefab
                // DestroyImmediate(spawnedObject);

            } catch (Exception ex) {
                Debug.LogError($"Error processing asset {assetId}: {ex}");
            }

            // Yield to Editor to avoid freezing
            await Task.Delay(50);
        }
...
// Helper to ignore the warning about "async void" / fire-and-forget
